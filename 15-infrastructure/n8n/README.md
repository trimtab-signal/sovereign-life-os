# n8n

> Self-hosted Zapier — visual workflows connecting all services. Native Home Assistant integration, HTTP nodes for any REST API.

**Website:** [https://n8n.io](https://n8n.io)

**Replaces:** Zapier ($228–588/yr)  
**Annual Savings:** $228–588/yr

## Quick Start

```bash
# 1. Copy and customize environment variables
cp .env.example .env
nano .env  # Edit passwords and settings

# 2. Start the service
docker compose up -d

# 3. Check logs
docker compose logs -f

# 4. Stop the service
docker compose down
```

## Notes

Visual workflow editor at http://localhost:5678. Connect all your services via HTTP Request nodes and native integrations.

## Data

All persistent data is stored in the `./data/` directory. Back this up regularly.

## Part of the Sovereign Life OS

This tool is part of the **Phenix Navigator Sovereign Life OS** — a complete open-source replacement for commercial SaaS services.
See the root README for the full deployment guide.
